...
import { WRITING_PROMPTS, WritingPrompt as PromptType, getRandomPrompt } from '@/data/writingPrompts';
			...
			<div className='border border-gray-200 rounded-md p-3 sm:p-4 bg-gray-50'>
				<div className='flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-0'>
					<div className='flex-1 sm:pr-4'>
						<p className='text-sm sm:text-base text-gray-700 font-light'>{currentPrompt ? `"${currentPrompt.text}"` : 'Loading prompt...'}</p>
					</div>
					<div className='flex items-center gap-2 flex-shrink-0'>
						...
						<Button variant='ghost' size='sm' onClick={handleRefreshPrompt} className='h-8 px-2' disabled={!currentPrompt}>
							<RefreshCw className='w-3 h-3' />
						</Button>

						{onHide && (
							<Button variant='ghost' size='sm' onClick={onHide} className='h-8 px-2'>
								<X className='w-3 h-3' />
							</Button>
						)}
					</div>
